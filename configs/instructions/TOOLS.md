...
- `palantir-mcp` — Foundry access (requires `FOUNDRY_TOKEN`).
...
| `palantir-mcp` | Disabled | — | Requires `FOUNDRY_TOKEN`; also via MCPorter |
...
- **Does NOT work with model proxy** (two blockers):
...
- Fixable with project header translation (~5 LOC) + exact model names. Not implemented; low priority.
