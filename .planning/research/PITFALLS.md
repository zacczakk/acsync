...
**What goes wrong:**
During pull (system → repo), real secret values in system files must be redacted back to `${VAR}` placeholders before writing to canonical files. Missing a single code path (e.g., a new MCP server, the `FOUNDRY_TOKEN` alias, or `provider_BEDROCK_API_KEY` in nested provider config) means a real API key gets committed.
...
- Nested paths like `provider.provider-bedrock.options.apiKey`
...
4. The `FOUNDRY_TOKEN` ↔ `FOUNDRY_TOKEN` alias must be a first-class mapping, not an afterthought.
...
3. Test fixture: OpenCode settings with both `{env:ANTHROPIC_BASE_URL}` and `${provider_BEDROCK_API_KEY}` — verify only the latter gets injected.
...
| `FOUNDRY_TOKEN` alias missed in redaction | `FOUNDRY_TOKEN` value committed under `FOUNDRY_TOKEN` key | First-class alias mapping: env key → secret name → `.env` variable |
