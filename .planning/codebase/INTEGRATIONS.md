...
- Palantir Foundry — Ontology access, dataset queries, builds
  ...
  - Auth: `FOUNDRY_TOKEN` env var (aliased as `FOUNDRY_TOKEN` in MCP env)
...
- provider Bedrock — network proxy to AWS Bedrock models
  - SDK/Client: `@ai-sdk/openai-compatible` npm package
  - Auth: `provider_BEDROCK_API_KEY` env var
  - Base URL: `${provider_BASE_URL_PROD}/model/`
  - Models: Claude Sonnet 4.5, Claude Haiku 4.5, Claude Opus 4.5
  - Config: `configs/common/settings/opencode.json` → `provider.provider-bedrock`

- provider Foundry — network proxy to Anthropic via model proxy
  ...
  - Config: `configs/common/settings/opencode.json` → `provider.provider-foundry`
...
| `provider_BEDROCK_API_KEY` | OpenCode settings | `provider.provider-bedrock.options.apiKey` |
| `FOUNDRY_TOKEN` | Palantir MCP | Aliased as `FOUNDRY_TOKEN` in MCP env block |
| `FOUNDRY_HOST` | Palantir MCP | CLI arg `--foundry-api-url` |
| `provider_BASE_URL_PROD` | OpenCode settings | `provider.provider-bedrock.options.baseURL` |
...
- `provider_BEDROCK_API_KEY` — Corporate AI proxy (Bedrock)
- `FOUNDRY_TOKEN` — Palantir Foundry access
- `FOUNDRY_HOST` — Palantir Foundry API URL
- `provider_BASE_URL_PROD` — Corporate AI proxy base URL
