...
<task type="auto">
  <name>Task 2: Rename directory, re-link binary, push to targets, clean stale files</name>
  <files>None (filesystem operations only)</files>
  <action>
Execute the following sequence (all commands from appropriate workdir):

1. **Commit all path changes from Task 1** before moving the directory:
   ```
...
2. **Rename the directory:**
   ...
   ```
   After this, ALL subsequent commands must use `workdir: ~/Repos/acsync`.

3. **Re-register the binary from new location:**
   ```bash
   # From ~/Repos/acsync:
   bun link
   ```
...
6. **Remove stale instruction files that push won't catch:**
   ```bash
...
<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 3: Verify repo rename and target propagation</name>
  <files>None</files>
  <action>
