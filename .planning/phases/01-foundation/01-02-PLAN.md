...
**src/formats/jsonc.ts** — JSONC with comment preservation per research:
...
- `modifyJsonc(existingText: string, path: (string | number)[], value: unknown): string` — uses `modify()` + `applyEdits()` from `jsonc-parser`. Formatting options: `{ tabSize: 2, insertSpaces: true, eol: '\n' }`. Returns the modified JSONC string with all comments preserved outside the edit path. Wraps errors in `ParseError`.
