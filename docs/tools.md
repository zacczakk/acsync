...
- `palantir-mcp`: Foundry access (Claude only; requires `FOUNDRY_TOKEN`).
...
- **Does NOT work with model proxy** out of the box (two blockers):
...
- Fixable: add `x-api-key` → `Bearer` header translation in project (~5 LOC) + use exact Foundry model names. Not implemented; low priority.
